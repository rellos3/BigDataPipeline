...
def get_data():
    ...
    conn = sqlite3.connect("/opt/airflow/dags/census_data_12.db")
...
def summarize_data():
    ...
    df[["ROOMS", "INCOME", "FAMILY_COUNT", "COMMUTE", "AGE"]].agg(
        ["mean", "median", "min", "max"]
    ).to_csv("summ_columns_2012.csv")

    df.groupby("SEX")[
        ["ROOMS", "INCOME", "FAMILY_COUNT", "COMMUTE", "AGE"]
    ].agg(["mean", "median", "min", "max"]).to_csv("gen_group_2012.csv")

    df.groupby("DISABILITY")[
        ["ROOMS", "INCOME", "FAMILY_COUNT", "COMMUTE", "AGE"]
    ].agg(["mean", "median", "min", "max"]).to_csv("dis_group_2012.csv")

    df.groupby("ROOMS")[["INCOME", "FAMILY_COUNT", "COMMUTE", "AGE"]].agg(
        ["mean", "median"]
    ).to_csv("room_group_2012.csv")


dag = DAG(
    "analyze_census_housing_2012",
    default_args={
