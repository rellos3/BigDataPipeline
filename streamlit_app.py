...
st.write("# Rooms")
st.bar_chart(roomsum)

incomesum = general_summ[["STATS", "INCOME"]]
incomesum.set_index('STATS', inplace=True)
st.write("# Income")
st.bar_chart(incomesum)

agesum = general_summ[["STATS", "AGE"]]
agesum.set_index('STATS', inplace=True)
st.write("# Age")
st.bar_chart(agesum)

COMMUTEsum = general_summ[["STATS", "COMMUTE"]]
...
gen_summ = pd.read_csv("gen_group.csv")
st.write(gen_summ)
